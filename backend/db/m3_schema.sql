...
CREATE TABLE IF NOT EXISTS m3_venues (
    ...
    venue_id    INT  NOT NULL UNIQUE,   -- M2 canonical venues.id — never generate locally
    venue_name  TEXT NOT NULL,
    area        TEXT,
    city        TEXT,
    types       JSONB,                  -- mirrors M2 venues.types (e.g. ["bar","restaurant"])
    display_tags  JSONB,               -- human labels, up to 5 (e.g. ["Brewpub", "Bar"])
    primary_type  TEXT,                -- display_tags[0] — used as AI venue_type framing
    cascade_types JSONB,               -- up to 3 specific non-generic types for archetype matching
    active      BOOLEAN NOT NULL DEFAULT TRUE,
    last_synced TIMESTAMPTZ,           -- timestamp of last successful sync from M2
...
-- Session-time Council suggestions (dashboard use — operator taps "get suggestion").
-- Stores the 5-part output in separate columns so parts can be queried individually,
-- fed back into the next session, and tracked by lever type.
-- =============================================================================
...
CREATE TABLE IF NOT EXISTS m3_se_suggestions (
    ...
    -- 5-part structured output (State → Mechanism → Lever → Action → Signal)
    state_text          TEXT,       -- what the room is doing right now
    mechanism_text      TEXT,       -- behavioral science why
    lever               TEXT CHECK (lever IN ('music','volume','lighting','staff','layout','temperature','verbal_mc')),
    action_text         TEXT,       -- specific instruction for the operator
    signal_text         TEXT,       -- what to watch for to confirm it worked
    raw_output          TEXT,       -- full Council synthesis kept for reference / Supabase logging
...
-- =============================================================================
-- SHOW PLANS
-- Pre-show plans generated by SE app 3-5 days before the show.
-- Not session-scoped — generated before a session exists.
-- Versioned: Agent 7 refinements each create a new version row.
-- Operator loads the finalized plan on show day — no regeneration needed.
-- =============================================================================

CREATE TABLE IF NOT EXISTS m3_show_plans (
    id                  BIGSERIAL PRIMARY KEY,
    venue_id            INT  NOT NULL,              -- M2 canonical venue_id
    plan_date           DATE NOT NULL,              -- the show date this plan targets
    version             INT  NOT NULL DEFAULT 1,    -- increments on each Agent 7 refinement
    phase_count         INT  NOT NULL,
    start_time          TEXT NOT NULL,              -- '21:00' (24h)
    end_time            TEXT NOT NULL,              -- '00:00' (24h, may be next day)

    -- Structured plan content
    phases              JSONB NOT NULL,             -- array of phase objects: label, start, end, bpm_range, chord, key, sub_bass, target, brainwave, watch_for[]
    transitions         JSONB,                      -- array of transition objects: from_phase, to_phase, instructions[]
    lighting_arc        JSONB,                      -- per-phase lighting: intensity, colour, movement
    staff_notes         TEXT,
    intervention_tree   JSONB,                      -- decision tree: if state X → do Y

    -- Generation metadata
    raw_output          TEXT,                       -- full Council synthesis
    generation_mode     TEXT CHECK (generation_mode IN ('council','fast')),
    council_debate_ref  TEXT,                       -- Supabase debate tree id
    m2_context_snapshot JSONB,                      -- snapshot of M2 venue profile used at generation time

    generated_at        TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    finalized           BOOLEAN NOT NULL DEFAULT FALSE,  -- TRUE = operator confirmed this version for show day

    UNIQUE (venue_id, plan_date, version)
);
...
CREATE INDEX IF NOT EXISTS idx_m3_se_suggestions_lever    ON m3_se_suggestions (lever, suggested_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_m3_show_plans_venue_date   ON m3_show_plans (venue_id, plan_date DESC);
CREATE INDEX IF NOT EXISTS idx_m3_show_plans_finalized    ON m3_show_plans (venue_id, plan_date, finalized);
